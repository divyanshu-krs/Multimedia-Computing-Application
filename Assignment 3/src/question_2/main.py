...
def parse_args():
    ...
    parser.add_argument('--docs', nargs='?', default='/home/div/32/MCA/Assignment 3/src/Problem_2/data/med/MED.ALL',
                        help='Path of the document file.')
    parser.add_argument('--queries', nargs='?', default='/home/div/32/MCA/Assignment 3/src/Problem_2/data/med/MED.QRY',
                        help='Path of the query file.')
    parser.add_argument('--gt', nargs='?', default='/home/div/32/MCA/Assignment 3/src/Problem_2/data/med/MED.REL',
...
def load_data(docs_path = "MED.ALL", query_path = "MED.QRY", gt_path = "MED.REL"):
...
def tf_idf(docs, queries, tokenizer):
    ...
    processed_docs = [d.lower().translate(table) for d in docs]
...
if __name__ == '__main__':
    ...
    if args.verbose:
        for i in range(sim_matrix.shape[1]):
            ...
            top.append(ranked_documents[:10]+1)
    ...
    rf_sim_matrix = relevance_feedback(vec_docs, vec_queries,gt, sim_matrix)
    if args.verbose:
    ...
    rfquery_sim_matrix = relevance_feedback_exp(vec_docs, vec_queries, rf_sim_matrix,gt)
    if args.verbose:
        for i in range(rfquery_sim_matrix.shape[1]):
            ranked_documents = np.argsort(-rfquery_sim_matrix[:, i])
            print ('Query:', i+1, 'Top relevant 10 documents:', ranked_documents[:10] + 1)
    evaluate_retrieval(rfquery_sim_matrix, gt, verbose=args.verbose)
